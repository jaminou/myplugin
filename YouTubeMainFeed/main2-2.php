...
function fetch_latest_video($channel_id) {
    ...
    // Extract video IDs (example: get the 5 most recent videos)
    $searchUrl = "https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=$channelId&order=date&maxResults=5&key=$apiKey";
    $searchResponse = wp_remote_get( $searchUrl );

    if ( ! is_wp_error( $searchResponse ) && 200 === wp_remote_retrieve_response_code( $searchResponse ) ) {
        $searchBody = wp_remote_retrieve_body( $searchResponse );
        $searchData = json_decode( $searchBody );
...
function get_youtube_embed_code($videoId) {
